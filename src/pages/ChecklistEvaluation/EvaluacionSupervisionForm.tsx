...
const EvaluacionSupervisionForm: React.FC<EvaluacionSupervisionFormProps> = ({ onCancel, evaluacionDraft }) => {
  ...
  const createEvaluacionMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Evaluación de supervisión creada exitosamente');
  ...
  const handleSaveDraft = (data: any) => {
    ...
    localStorage.setItem(`supervision-borrador-${borradorId}`, JSON.stringify({
      subcriteriosRatings,
      selectedColaborador,
  ...
  const handleFinish = (data: any) => {
    ...
    if (evaluacionDraft?.id) {
      localStorage.removeItem(`supervision-borrador-${evaluacionDraft.id}`);
    }
    ...
    if (evaluacionDraft?.id) {
      ...
      fetch(`/evaluaciones/${evaluacionDraft.id}`, {
        ...
        toast.success('Evaluación de supervisión finalizada exitosamente');
        queryClient.invalidateQueries({ queryKey: ['evaluaciones-evaluador'] });
        onCancel();
      }).catch((error: any) => {
  ...
  React.useEffect(() => {
    if (evaluacionDraft?.id) {
      ...
      const borrador = localStorage.getItem(`supervision-borrador-${evaluacionDraft.id}`);
      if (borrador) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Evaluación de Supervisión Docente</h1>
          <p className="text-muted-foreground">Evaluación supervisora usando criterios predefinidos.</p>
        </div>
      </div>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleFinish)} className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-orange-50 to-red-50 dark:from-orange-950/30 dark:to-red-950/30 p-6 rounded-lg border border-orange-200 dark:border-orange-800">
            <h3 className="font-bold text-lg mb-3 text-orange-900 dark:text-orange-100">Escala de Valoración</h3>
          ...
          {Object.entries(criteriosAgrupados).map(([criterioNombre, subcriteriosGrupo]) => (
            <Card key={criterioNombre} className="border-l-4 border-l-orange-500">
              <CardHeader className="bg-gradient-to-r from-orange-50/50 to-red-50/50 dark:from-orange-950/20 dark:to-red-950/20">
                <CardTitle className="text-lg text-orange-700 dark:text-orange-300">{criterioNombre}</CardTitle>
              </CardHeader>
                      ...
                      <div className="flex items-start justify-between gap-6">
                        <div className="flex-1">
                          <Label className="text-base font-medium leading-relaxed cursor-pointer">
                            <span className="inline-flex items-center justify-center w-6 h-6 bg-orange-500 text-white rounded-full text-sm font-bold mr-3">
                              {index + 1}
                            </span>
                            {subcriterio.texto}
                          </Label>
          ...
          <Card className="border-2 border-orange-500">
            ...
            <CardContent className="p-6">
              <div className="text-4xl font-bold text-center text-orange-600">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormControl>
                      <Textarea
                        placeholder="Observaciones sobre el desempeño del docente, fortalezas identificadas y recomendaciones de mejora..."
